...
export default function ScheduleSettings() {
  ...
  if (!config) {
    return <p className="text-sm text-gray-400">{error ?? '불러오는 중...'}</p>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-gray-700">자동 크롤링 활성화</span>
        <button
      ...
      <div className="flex items-center gap-3">
        <span className="text-sm font-medium text-gray-700 w-20 shrink-0">실행 시간</span>
        <select
          value={config.hour}
      ...
      {/* 수집 토픽 */}
      <div>
        <span className="text-sm font-medium text-gray-700 block mb-2">수집 토픽</span>
      ...
      {/* 토픽당 편수 */}
      <div className="flex items-center gap-3">
        <span className="text-sm font-medium text-gray-700 w-20 shrink-0">토픽당 편수</span>
        <input
          type="number"
          min={1}
          max={20}
        ...
        />
        <span className="text-xs text-gray-400">편 (최대 20)</span>
      </div>

      {/* 수집 소스 */}
      <div>
        <span className="text-sm font-medium text-gray-700 block mb-2">수집 소스</span>
      ...
      {/* 요약 모델 */}
      <div>
        <span className="text-sm font-medium text-gray-700 block mb-2">요약 모델</span>
        <div className="flex flex-wrap gap-4">
          {CLAUDE_MODELS.map(m => (
            <label key={String(m.id)} className="flex items-center gap-1.5 text-sm cursor-pointer">
              <input
                type="radio"
